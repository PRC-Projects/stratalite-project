...
import { Star, MapPin, Trophy, ShieldCheck, ChevronDown, Search, XCircle } from "lucide-react";
...
export default function FreelancerCategoryPage() {
  ...
  const handleViewProfile = (profileId: number) => {
     router.push(`/login?callbackUrl=/freelancers/${params.category}/${profileId}`);
  };
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 pb-12">
        ...
        <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 mb-8">
           <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              <div>
                <h1 className="text-3xl font-bold text-gray-800 capitalize mb-2">Hire the best {categoryName}</h1>
                <p className="text-gray-500">Browse {filteredFreelancers.length} {categoryName} with verified track records.</p>
              </div>
              <button className="px-4 py-2 bg-blue-600 text-white rounded-full font-bold text-sm hover:bg-blue-700">Post a Job</button>
           </div>
        </div>
           ...
           <aside className="w-full lg:w-64 flex-shrink-0 hidden lg:block space-y-6">
              <div className="flex justify-between items-center mb-2">
                 ...
                 {(selectedRateRanges.length > 0 || selectedBadges.length > 0) && (
                   <button onClick={() => { setSelectedRateRanges([]); setSelectedBadges([]); }} className="text-xs text-blue-600 hover:underline">Reset</button>
                 )}
              </div>

              {/* Hourly Rate */}
              <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm">
                 <h4 className="font-bold text-gray-800 mb-3 text-sm">Hourly Rate</h4>
                 <div className="space-y-2">
                    {["$10 and below", "$10 - $30", "$30 - $60", "$60 & above"].map((range) => (
               ...
               {/* Talent Quality */}
               <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm">
                 <h4 className="font-bold text-gray-800 mb-3 text-sm">Talent Quality</h4>
                 <div className="space-y-2">
                    {["Top Rated Plus", "Top Rated", "Rising Talent"].map((badge) => (
                      <label key={badge} className="flex items-center gap-2 text-sm text-gray-600 cursor-pointer hover:text-gray-900">
                         <input 
                            type="checkbox" 
                            className="rounded text-blue-600 focus:ring-blue-500 w-4 h-4 border-gray-300"
           ...
           <div className="flex-1 space-y-4">
              ...
              {visibleFreelancers.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-20 bg-white rounded-xl border border-dashed border-gray-300">
                    ...
                    <h3 className="text-lg font-bold text-gray-600">No freelancers found</h3>
                    <button onClick={() => {setSearchQuery(""); setSelectedBadges([]); setSelectedRateRanges([])}} className="mt-4 text-blue-600 font-medium hover:underline">Clear all filters</button>
                </div>
              ) : (
                       ...
                       <div className="flex flex-col md:flex-row gap-6">
                          <div className="flex-shrink-0 relative">
                             <div className="relative w-16 h-16 md:w-20 md:h-20">
                                <img src={freelancer.img} alt={freelancer.name} className="w-full h-full object-cover rounded-full" />
                          ...
                          <div className="flex-1">
                             <div className="flex flex-col md:flex-row md:justify-between items-start mb-1">
                                <div>
                                   <h3 className="text-lg font-bold text-blue-600 hover:underline">{freelancer.name}</h3>
                                   <p className="font-medium text-gray-800 text-sm mb-1">{freelancer.title}</p>
                                </div>
                                <div className="mt-2 md:mt-0">
                                  <button 
                                    onClick={(e) => { e.stopPropagation(); handleViewProfile(freelancer.id); }} 
                             ...
                             <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-xs md:text-sm text-gray-600 mb-3 font-medium">
                                <span className="font-bold">{freelancer.rateStr}</span>
                                <span className="text-gray-300">|</span>
                                <span className="text-gray-600">{freelancer.earned}</span>
                                <span className="text-gray-300">|</span>
                                <div className="flex items-center gap-1 text-blue-700 bg-blue-50 px-2 py-0.5 rounded-full">
                                   <Trophy size={12} /> {freelancer.jobSuccess}% Job Success
                                </div>
                                {freelancer.badge && (
                             ...
                             <p className="text-sm text-gray-600 mb-4 leading-relaxed line-clamp-2">{freelancer.bio}</p>

                             <div className="flex flex-wrap gap-2">
                                {freelancer.skills.map((skill, i) => (
                                  <span key={i} className="px-3 py-1 bg-gray-100 text-gray-600 text-xs rounded-full hover:bg-gray-200 transition-colors">
                                    {skill}
                                  </span>
                                ))}
                             </div>
